...
Env: ORGANICA_AI_KEY (optional, for the reasoning service), ALERT_WEBHOOK (optional
Slack/webhook URL), MONITOR_DATA (csv path).
"""
...
# Which stack this scheduler runs. It runs the commercial one: revenue quality, unit
# economics, pricing, scenario, self-validation, benchmark, valuation. There is no credit path
# in it. Switched on against a facility it would compute the wrong things and alert on them,
# with nothing in the output saying so. It therefore refuses rather than guesses.
STACK = os.environ.get("MONITOR_STACK", "commercial").strip().lower()
if STACK != "commercial":
    sys.stderr.write(
        f"monitor: MONITOR_STACK={STACK!r} is not wired. This scheduler runs the commercial "
        f"stack only (revenue quality, unit economics, pricing, scenario, validation, "
        f"benchmark, valuation). The credit stack — covenants, headroom, distance to breach — "
        f"is not reachable from here, so running it would alert on the wrong numbers. "
        f"Refusing to start.\n")
    raise SystemExit(2)
