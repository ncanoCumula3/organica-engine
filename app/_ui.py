...
import credit as CR, icpaper as IPR, holdmonitor as HM, creditrisk as CXR
...
.stButton button{{border:1px solid {LINE};border-radius:2px;font-weight:500}}
    ...
    NAV=["Business cases","Ask","Journeys","Connect data","Data","Dashboard","Deliverability","IC paper","Analyses","Export"]
    ...
    if page=="Business cases":
        ...
        R=ss.get("results") or run_all(ss["df"]); a=ss["assump"]; t=ss["terms"]
        ...
        with tab_live:
            if ts is None:
                ...
                pinned=HM.approval_terms(ts, ss["df"], appr, a, t)
        ...
        try:
            _secs=IPR.build(R, df=ss["df"], a=ss["assump"], terms=ss["terms"], company=comp,
                            stamp=stamp, journeys_mod=J, prov_mod=PV,
                            risk=_risk_pack(R, ss["assump"], ss["terms"]))
